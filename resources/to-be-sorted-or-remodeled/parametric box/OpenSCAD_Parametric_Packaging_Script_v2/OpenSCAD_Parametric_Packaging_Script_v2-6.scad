...
//	["B", "Rectangle",	[2,-2,0,+box_b/2,10,"outside"],		[2,4,5]		]//aligned to top centre
	["N", "Rectangle",		[0, -5.25,0,0,0,"inside"],				[wall_t, 11.25, 8]			],
	["W", "Rectangle",		[6.75, -4.55,0,0,0,"outside"],				[wall_t/2, 5.5, 5.3]			],
...
//	["N", "Cylinder",	[-1,8,0,0,0,"inside"],					[6,3,10]		],//for comparison
	...
	["T", "Cylinder",	[11,14.4,0,0,0,"inside"],					[5,3,6]		],//fail
	["T", "Cylinder",	[11,-14.4,0,0,0,"inside"],					[5,3,6]		],
	["T", "Cylinder",	[-11,-14.4,0,0,0,"inside"],				[5,3,6]		],//fail
	["T", "Cylinder",	[-11,14.4,0,0,0,"inside"],					[5,3,6]		],
	["B", "Nub_Post",	[-11,-14.4,0,0,0,"inside"],				[4.3,2.5,0.5,1.7,6]		],
	["B", "Nub_Post",	[11,14.4,0,0,0,"inside"],				[4.3,2.5,0.5,1.7,6]		],
	["B", "Nub_Post",	[11,-14.4,0,0,0,"inside"],				[4.3,2.5,0.5,1.7,6]		],
	["B", "Nub_Post",	[-22/2,28.8/2,0,0,0,"inside"],			[4.3,2.5,0.5,1.7,6]		]//for comparison//
	];
...
text = [//recessed text on faces [face_name, text_to_write, shape_position[x_pos,y_pos,x_offs,y_offs,rotate,align], shape_size[depth,font_height,font_spacing,mirror]] Note: for silly reasons mirror must be 0 or 1 corresponding to false and true in this version
	...
	["N", "X+",			[0,9,0,0,0,"outside"], 					[1,10,1,0]],
	["S", "X-",			[0,-6,0,0,0,"outside"], 					[1,10,1,0]],
	["E", "Y-",			[0,-6,0,0,0,"outside"], 					[1,10,1,0]],
	["W", "Y+",			[0,9,0,0,0,"outside"], 					[1,10,1,0]],
	["T", "Z+",			[0,0,0,0,90,"outside"], 					[1,10,1,0]],
...
module make_box(box, corner_radius=3, corner_sides=5, lip_h=2, lip_fit=0, top_bottom_ratio=0.5,mouse_ears=[false],layout="beside", separation=2, holes=[], posts=[], text=[], items=[]){
	...
	translate(v=[0,0,box[2]/2]){
		if (layout=="beside"){
			echo("beside");
			union(){
				translate(v=[(separation+box[0])/2,0,0]) half_box(box, corner_radius, corner_sides, lip_h, lip_fit, top_bottom_ratio, which_half="bottom", holes=holes, posts=posts, mouse_ears=mouse_ears, text=text, items=items);
				translate(v=[-(separation+box[0])/2,0,0]) rotate(a=[0,180,0]) half_box(box, corner_radius, corner_sides, lip_h, lip_fit, top_bottom_ratio, which_half="top", holes=holes, posts=posts, mouse_ears=mouse_ears, text=text, items=items);//cube(size=[box[0],box[1],box[2]],center=true);
			}
			...
			half_box(box, corner_radius, corner_sides, lip_h, lip_fit, top_bottom_ratio, which_half="top", holes=holes, posts=posts, mouse_ears=[false], text=text, items=items);
		}else if (layout=="top"){
			echo("top");
